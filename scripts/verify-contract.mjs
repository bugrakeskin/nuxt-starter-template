...
const migrationStep = previewWorkflow.steps.find(step => step.name === 'migrate-preview')
assert.equal(migrationStep.image, 'registry.unfogy.com/${CI_REPO_NAME}/app:${CI_COMMIT_SHA}')
assert.equal(migrationStep.dns, '10.77.30.1')
assert.equal(migrationStep.environment.SUPABASE_DB_URL.from_secret, 'preview_database_url')
assert.deepEqual(migrationStep.commands, ['node /app/.output/migrate.mjs'])
const deployStep = previewWorkflow.steps.find(step => step.name === 'deploy-preview')
assert.ok(previewWorkflow.steps.indexOf(previewScan) < previewWorkflow.steps.indexOf(migrationStep))
assert.ok(previewWorkflow.steps.indexOf(migrationStep) < previewWorkflow.steps.indexOf(deployStep))
assert.equal(deployStep.dns, '10.77.30.1')
assert.equal(deployStep.environment.COOLIFY_DEPLOY_TOKEN.from_secret, 'coolify_preview_deploy_token')
